...
public class ExampleConditionalSum {
	...
	public static void main(String[] args) {
		...
		int M = 7; // 7 items
		int N = 3;// 3 bins (categories)
		int[] w = {3,4,2,5,7,1,2};// weight of items
		
		VarIntLS[] X = new VarIntLS[M];
		for(int i = 0; i < M; i++) X[i] = new VarIntLS(mgr,0,N-1);
		IFunction f = new ConditionalSum(X, w, 1);// consider bin 1 
		mgr.close();
	
		X[0].setValuePropagate(0);	X[1].setValuePropagate(2);
		X[2].setValuePropagate(2);	X[3].setValuePropagate(1);
		X[4].setValuePropagate(0);	X[5].setValuePropagate(2);

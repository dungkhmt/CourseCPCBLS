...
public class MinMaxSelector {
	...
	public MinMaxSelector(IFloatConstraint S) {
		_S = S;
		...
		for (int i = 0; i < _vars.length; i++) {
			if (_minValue > _vars[i].getMinValue())
				_minValue = _vars[i].getMinValue();
			if (_maxValue < _vars[i].getMaxValue())
				_maxValue = _vars[i].getMaxValue();
		}
	}

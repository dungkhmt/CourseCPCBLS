...
//		while (countBin < numBin) {
			...
			System.out.println(bin +"\t" +bins[bin].getCapacity());
...
//			System.out.println(bin);
			...
			if (w < bins[bin].getMinLoad() || bins[bin].getCapacity() < bins[bin].getMinLoad()
					|| bins[bin].getP() < 0 || bins[bin].getCapacity() < 0) {
				// iter.remove();
				countBin++;
				binTake[bin] = true;
				continue;
	...
	public ArrayList<Integer> loadUpdateBinItem(String binfile) {
		ArrayList<Integer> binUSe = new ArrayList<>();
		Scanner sc;
		try {
			sc = new Scanner(new File(binfile));
			int temp = 0;
			while (temp != -1) {
				temp = sc.nextInt();
				binUSe.add(temp);
	...
	public static void main(String[] args) {
		...
		String path1 = "src/khmtk60/miniprojects/multiknapsackminmaxtypeconstraints/model/MinMaxTypeMultiKnapsackInput-3000.json";
		MinMaxTypeMultiKnapsackSolution minMaxType = new MinMaxTypeMultiKnapsackSolution();
		minMaxType.readData(path1);
		minMaxType.GA(minMaxType.numI, popSize, 5000, 0.3, 0.8, rand);

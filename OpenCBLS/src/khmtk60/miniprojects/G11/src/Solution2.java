...
public class Solution2 {
	...
	public void initialize() {
		...
		for (Item item: this.items) {
			while (true) {
				Bin bin = this.bins.get(rand.nextInt(m));
				if (!bin.violateIfAddItem(item)) {
					this.assignValue(item, bin);
					break;
				}
			}
		}
	...
	public void printOutput(String fileName) {
		...
		String directory = new File(".").getAbsolutePath() + "/src/khmtk60/miniprojects/multiknapsackminmaxtypeconstraints/";
	...
	public RemoveBinMove moveBinToOtherBin() {
		Bin binToRemove = this.chooseBin();
		Random rand = new Random();
...
//		}
	...
	public void testInput() {
		this.bins.sort((Bin b1, Bin b2) -> (b1.lw - b2.lw < 0) ? 1 : (b1.lw == b2.lw ? 0 : -1));
		for (Bin bin: this.bins) {
			System.out.println(bin.index + " " + bin.lw + "-" + bin.w);
		}
	}
	...
	public static void main(String[] args) {
		...
		demo.readInput("MinMaxTypeMultiKnapsackInput-1000.json");
//		demo.testInput();
		demo.initialize();
		demo.search();
		demo.printOutput("MinMaxTypeMultiKnapsackOutput-1000.json");
	}

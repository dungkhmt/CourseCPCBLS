...
public class TabuSearch {
				...
				for (int v = x[i].getMinValue(); v <= x[i].getMaxValue(); v++) {
					...
					if (deltaS <= 0)
						if (tabu[i][v - minV] <= it
							...
							minDelta = getMinDelta(x, moves, minDelta, i, v, deltaF);
						}
				}
			}
	...
	public void search(IConstraint S, int tabulen, int maxTime, int maxIter,
		...
		while (it < maxIter && System.currentTimeMillis() - t0 < maxTime
			...
			for (int i = 0; i < n; i++) {
				for (int v = x[i].getMinValue(); v <= x[i].getMaxValue(); v++) {
					...
					if (tabu[i][v - minV] <= it
						...
						minDelta = getMinDelta(x, moves, minDelta, i, v, delta);
					}
				}
			}
			...
			if (moves.size() <= 0) {
				...
				// perform the move
				OneVariableValueMove m = moves.get(R.nextInt(moves.size()));
				int sel_i = map.get(m.getVariable());
				sel_v = m.getValue();
				x[sel_i].setValuePropagate(sel_v);
				tabu[sel_i][sel_v - minV] = it + tabulen;
	...
	private int getMinDelta(VarIntLS[] x, ArrayList<OneVariableValueMove> moves, int minDelta, int i, int v, int delta) {
		int sel_i;
		int sel_v;
		if (delta < minDelta) {
			minDelta = delta;

...
import khmtk60.miniprojects.multiknapsackminmaxtypeconstraints.model.MinMaxTypeMultiKnapsackSolution.AssignMove;
	...
	public void greedyInitializeX() {
		...
		for (int i = 0; i < n; i++) {
			...
			for (int bin : binDomain) {
				if (current_w_bin[bin] + w[i] <= capacity[bin] && current_p_bin[bin] + p_item[i] <= p_bin[bin]) {

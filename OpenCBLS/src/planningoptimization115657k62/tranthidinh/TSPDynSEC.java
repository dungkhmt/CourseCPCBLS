...
public class TSPDynSEC {
    ...
    private void createFlowConstraint(){
        // dieu kien luong: tong so luong di vao = 1, tong so luong di ra =1
        for(int i = 0; i < N; i++){
    ...
    public void solveDynamicAddSubTourConstraint(){
        ...
        while(!found){
            ...
            if (resultStatus != MPSolver.ResultStatus.OPTIMAL) {
                System.err.println("not have an optimal solution");
                return;
            }
            ...
            for(int s = 0; s < N; s++)if(!mark[s]){
                ...
                if(C.size() < N){// phat hien chu trinh con
                    System.out.print("phat hien chu trinh con, C = ");

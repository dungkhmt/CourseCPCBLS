import com.google.ortools.linearsolver.MPConstraint;
import com.google.ortools.linearsolver.MPObjective;
import com.google.ortools.linearsolver.MPSolver;
import com.google.ortools.linearsolver.MPSolverResponseStatus;
import com.google.ortools.linearsolver.MPVariable;

public class MIPChap2 {
	
	static {
		System.loadLibrary("jniortools");
		
	}
	
	public static void sloveMIP() {
		double INF = java.lang.Double.POSITIVE_INFINITY; // gán giá trị vô cùng
		MPSolver solver = new MPSolver("SimpleMIP", MPSolver.OptimizationProblemType.CBC_MIXED_INTEGER_PROGRAMMING);
		MPVariable x1 = solver.makeNumVar(0, 14, "x1"); //khai báo x1 vơi 0<= x1 <= 14
		MPVariable x2 = solver.makeIntVar(0, 20, "x2"); // Khai báo x2 với 0 <= x2 <= 20
		
		
		//x1 - 10x2 < 7
		MPConstraint c1 = solver.makeConstraint(-INF, 7);
		c1.setCoefficient(x1, 1);
		c1.setCoefficient(x2, -10);
		
		//0 < 2x1 + 3x2 < 20
		MPConstraint c2 = solver.makeConstraint(0, 20);
		c1.setCoefficient(x1, 2);
		c1.setCoefficient(x2, 3);
		
		// Hàm mục tiêu x1 + x2
		MPObjective obj = solver.objective();
		obj.setCoefficient(x1, 1);
		obj.setCoefficient(x2, 1);
		obj.setMaximization();
		
		MPSolver.ResultStatus rs = solver.solve();
		if(rs!= MPSolver.ResultStatus.OPTIMAL) {
			System.out.print("Cannot find optimal solution");
		}
		else {
			System.out.println("Objective value = " + obj.value());
			System.out.println("x1 = " + x1.solutionValue());
			System.out.println("x2 = "+ x2.solutionValue());
		}
	}

	public static void main(String[] args) {
		sloveMIP();

	}

}

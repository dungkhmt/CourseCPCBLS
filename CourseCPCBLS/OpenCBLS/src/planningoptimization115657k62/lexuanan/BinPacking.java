...
public class BinPacking {
	...
	public void buildModel() {
		...
		for (int i = 0; i < N; i++) {
			Constraint c1 = model.arithm(model.intOffsetView(x[i], h[i]),"<=",W);
			Constraint c2 = model.arithm(model.intOffsetView(y[i], w[i]),"<=",H);
			model.ifThen (model.arithm(o[i],"=",1), model.and(c1,c2) );
		}
		for (int i = 0; i < N - 1; i ++) {
			for (int j = i + 1; j < N; j ++) {

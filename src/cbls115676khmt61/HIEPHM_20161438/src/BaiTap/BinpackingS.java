...
public class BinpackingS {
    ...
    public void stateModel() {
        ...
        //add constrain lay theo thu tu
        for(int i=0;i<N-1;i++){
            IConstraint[] constraints = new IConstraint[2];
            constraints[0] = new LessOrEqual(x[i+1],new FuncPlus(x[i],w[i]));
            constraints[1] = new LessOrEqual(y[i+1],new FuncPlus(y[i],h[i]));
            IConstraint ct = new AND(constraints);
            S.post(ct);
        }
        mgr.close();
    }
    ...
    public static void main(String[] args) {
        BinpackingS binpacking = new BinpackingS();
        binpacking.solve();
    }
}

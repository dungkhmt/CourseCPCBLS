...
public class BinPacking {
    ...
    int K = 5;
    VarIntLS X[][]; // NxK
    ArrayList<ArrayList<Integer>> C = new ArrayList<>(); // các dung dịch không được phép chứa cùng thùng

    int dungtich[] = {20, 15, 10, 20, 20, 25, 30, 15, 10, 10, 20, 25, 20, 10, 30, 40, 25, 35, 10, 10}; //dung tich moi dung dich
    int v[] = {60, 70, 90, 80, 100}; // dung tich moi thung
    ...
    public void buildModel() {
        ...
        //Cac dung dich deu duoc dong thung
        for (int j = 0; j < N; j++) {
        ...
        //Cac dung dich cam khong dong thung chung
        ...
        //Dung tich khong vuot qua
        for (int k = 0; k < K; k++) {
            VarIntLS[] Y = new VarIntLS[N];
            for (int j = 0; j < N; j++)
                Y[j] = X[j][k];
            S.post(new LessOrEqual(new ConditionalSum(Y, dungtich, 1), v[k]));
        }
        mgr.close();
    }

...
public class MiniProject {
	...
	public void print() {
		for (int i = 1; i <= K; i++) {
			System.out.println("\tD[" + i + "] = " + D[i-1].getValue() + ", C[" + i + "] = " + C[i-1].getValue() + 
					", T[" + i + "] = " + T[i-1].getValue());
		}
	...
	public void exploreNeighborhood1(ArrayList<Move> candicate) {
		candicate.clear();
	...
	public void exploreNeighborhood2(ArrayList<Move> candicate) {
		candicate.clear();
		int minDeltaC = Integer.MAX_VALUE;
		double minDeltaF = minDeltaC;
		for (int k = 1; k < XR.getNbRoutes(); k++) {
			for (Point y = XR.next(XR.startPoint(k)); y != XR.endPoint(k); y = XR.next(y)) {
				for (Point x: clientPoint) {
					if (x != y && x != XR.next(y)) {
						int deltaC = S.evaluateTwoPointsMove(x, y);
						double deltaF = obj.evaluateTwoPointsMove(x, y);
						// if (!(deltaC < 0 || deltaC == 0 && deltaF < 0)) continue;
						if (deltaC < minDeltaC || deltaC == minDeltaC && deltaF < minDeltaF) {
							candicate.clear();
							candicate.add(new Move(x, y));
	...
	public void search(int maxIter) {
		...
		while (it < maxIter) {
			exploreNeighborhood1(candicate);
			if (candicate.size() <= 0) {
	...
	public void search2(int maxIter) {
		...
		ArrayList<Move> candicate = new ArrayList<Move>();
		while (it < maxIter) {
			exploreNeighborhood2(candicate);
			if (candicate.size() <= 0) {
				System.out.println("Reach local optimum");
				break;
			}
			...
			System.out.println("Step "+ it + ":\n" + XR.toString() + "violations = " + S.violations() + ", y = " + obj.getValue());
//			print();
			it++;
		}
	}

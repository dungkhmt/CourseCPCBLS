...
//				if(cand.size()==0) {
				int delta=c.getAssignDelta(x[i],v);// tra ve su thay doi violation khi x[i]
	...
	public void search(IConstraint c, int maxInter) {// maxInter: so vong lap
		this.x=c.getVariables();
		int it=0;
		Random R=new Random();
		ArrayList<Move> cand= new ArrayList<Move>();
		while(it<maxInter && c.violations()>0) {//violation: so bien khong thoa man rang buoc
			exploreNeighborhood(c,cand);
			if(cand.size()==0) {

...
public class GraphPartitioningCBLS {
	...
	public void buildModel(){
		...
		f = new GraphPartitioningCost(c, x); // objective function
		mgr.close();
	}
	...
	public void search1(int maxIter){
		...
		*/
		...
		while(it < maxIter){
			...
			for(int i = 0; i < N; i++){
				for(int j = i+1; j < N; j++) 
					if(x[i].getValue() != x[j].getValue()){ // check có cùng 1 partition không
	...
	public static void main(String[] args) {
		...
		String fileName = "graph_14_39.txt";
...
//		app.runExpr();
		...
		for (int i = 0; i < 14; i++) {
			System.out.print(app.x[i].getValue()+"\t");
		}
	}

...
public class SportScheduling {
	...
	public void buildModel() {
			...
			for(int t = 1;t <= T;t++) {
				...
				for(int j = 0;j < N;j++) {
					if(i != j) {
						for(int k = 0;k < N;k++) {
							if(k != j) {
								c4.setCoefficient(F[i][j][k][t], 1);
								c4.setCoefficient(Y[i][j][t-1], -1);
							}
						}
					}
				}
		...
		for(int i = 0;i < N;i++) {
			...
			c3.setCoefficient(D[i], 1);
			for(int j = 0;j < N;j++) {
				for(int k = 0;k < N;k++) {
					if(j != k) {
						for(int t = 1;t <= T;t++) {
							c3.setCoefficient(F[i][j][k][t], -d[j][k]);
						}
					}
				}
			}
		...
		for (int i = 0; i < N; i++) {
			for(int j = 0;j < N;j++) {
				for(int k = 0;k < N;k++) {
					if(j != k) {
						for(int t = 1;t <= T;t++) {
							obj.setCoefficient(F[i][j][k][t], d[j][k]);
						}
					}
				}
			}

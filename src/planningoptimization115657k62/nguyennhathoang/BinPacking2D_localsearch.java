...
public class BinPacking2D_localsearch {
	...
	public void init() {
		...
		for (int i = 0; i < N; i++) {
			CS.post(new Implicate(new IsEqual(x[i][2], 0),
					new AND(new LessOrEqual(x[i][0], W - w[i]), new LessOrEqual(x[i][1], H - h[i]))));
			CS.post(new Implicate(new IsEqual(x[i][2], 1),
					new AND(new LessOrEqual(x[i][0], H - w[i]), new LessOrEqual(x[i][1], W - h[i]))));
		}
